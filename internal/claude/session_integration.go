...
// SessionAwareIntegration manages Claude Code with proper session handling
type SessionAwareIntegration struct {
	execPath       string
...
// NewSessionAwareIntegration creates a new session-aware Claude integration
func NewSessionAwareIntegration(execPath string) *SessionAwareIntegration {
	if execPath == "" {
		execPath = "claude"
	}
	
	return &SessionAwareIntegration{
		execPath:       execPath,
...
func (sai *SessionAwareIntegration) SendMessageInSession(sessionID, message, contextPrefix string) (*ClaudeResponse, error) {
	sai.mu.Lock()
	session, exists := sai.activeSessions[sessionID]
	if !exists {
		...
		sai.activeSessions[sessionID] = session
	}
	sai.mu.Unlock()
	...
	if session.MessageCount == 0 {
		...
		cmd = exec.Command(sai.execPath, "--print", "--output-format", "json", fullMessage)
	} else if session.ClaudeSessionID != "" {
		// Resume specific session with session ID
		fmt.Printf("[DEBUG] Resuming session: %s\n", session.ClaudeSessionID)
		cmd = exec.Command(sai.execPath, "--print", "--output-format", "json", "--resume", session.ClaudeSessionID, fullMessage)
	} else {
	...
	response, sessionInfo, err := sai.processJSONResponse(cmd)
	if err != nil {
...
func (sai *SessionAwareIntegration) ResumeSession(sessionID, claudeSessionID string) error {
	sai.mu.Lock()
	defer sai.mu.Unlock()
	
	// Verify the session exists in Claude
	cmd := exec.Command(sai.execPath, "--print", "--output-format", "json", "--resume", claudeSessionID, "Session resumed")
	...
	sai.activeSessions[sessionID] = &ManagedSession{
		SessionID:       sessionID,
...
func (sai *SessionAwareIntegration) GetSession(sessionID string) *ManagedSession {
	sai.mu.RLock()
	defer sai.mu.RUnlock()
...
func (sai *SessionAwareIntegration) processStreamingResponse(cmd *exec.Cmd) (*ClaudeResponse, *SessionInfo, error) {
...
func (sai *SessionAwareIntegration) processJSONResponse(cmd *exec.Cmd) (*ClaudeResponse, *SessionInfo, error) {
	output, err := cmd.CombinedOutput()
	if err != nil {
...
func (sai *SessionAwareIntegration) startNewClaudeSession() (string, error) {
	// No longer needed - session will be created on first real message
	return "", nil
}

// GetActiveSession returns the active session for a given ID
func (sai *SessionAwareIntegration) GetActiveSession(sessionID string) (*ManagedSession, bool) {
	sai.mu.RLock()
	defer sai.mu.RUnlock()
...
func (sai *SessionAwareIntegration) CloseSession(sessionID string) {
	sai.mu.Lock()
	defer sai.mu.Unlock()
	
	delete(sai.activeSessions, sessionID)
}

// InteractiveChatWithMemory runs a session-aware chat with memory
func (sai *SessionAwareIntegration) InteractiveChatWithMemory(sessionManager *memory.SessionManager) error {
	...
	for {
		...
		// Show session info
		if session, exists := sai.GetActiveSession(sessionID); exists {
			fmt.Printf("📊 Session: %s (msg #%d)\n", session.ClaudeSessionID[:8], session.MessageCount)
		}
	}
	
	// Get final session info for continuation
	if session, exists := sai.GetActiveSession(sessionID); exists && session.ClaudeSessionID != "" {
...
func (sai *SessionAwareIntegration) SendMessageWithMemory(message, memoryContext string) (*memory.ClaudeResponse, error) {
	// Use a default session for compatibility
	sessionID := "default"
	
	claudeResp, err := sai.SendMessageInSession(sessionID, message, memoryContext)
	if err != nil {
		return nil, err
	}
